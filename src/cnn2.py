...
def run_cnn():
    ...
    X1 = numpy.vstack(train_features)
    T1 = numpy.vstack(train_labels)
    training_set = DataSet(X1, T1)
    X2 = numpy.vstack(test_features)
    T2 = numpy.vstack(test_labels)
    validation_set = DataSet(X2, T2)

    # Train for 30 episodes (with tuned parameters for MBSGD)
    optimizer = MBSGD({"maximal_iterations": 1}, learning_rate=0.05,
    ...
    print("Correct predictions on training set: %d/%d, and percent is: %f, and accuracy is %f"
          % (classification_hits(net, training_set), len(X1), training_percent, accuracy(net, training_set)))
    print("Confusion matrix:")
    print(confusion_matrix(net, training_set))
    print("Correct predictions on test set: %d/%d, and percent is: %f, and accuracy is %f"
          % (classification_hits(net, validation_set), len(X2), testing_percent, accuracy(net, validation_set)))
    print("Confusion matrix:")
